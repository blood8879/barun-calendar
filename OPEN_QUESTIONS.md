...
## 0. [출시 전 필수 · 신규 2026-08-13] 경쟁 앱 `kr.co.neoandroid.neolunar` 대응

> **상태: 경쟁 실측에서 새로 발견됨. 리서치 문서(`app-research-2026-07.md`) 15R에 기록.**
>
> `음력 달력·기념일 - 간지·24절기·대체휴일`(`kr.co.neoandroid.neolunar`)
> — **다운로드 10만+ / 리뷰 1,250 / 광고+인앱결제 / 2026-08-09 갱신**.
> 이 앱은 9R 경쟁 조사 표에 **누락돼 있었고**, 스토어 설명이 바른달력 기능 명세와 거의 1:1이다:
> *"음력 날짜는 **한국천문연구원(KASI) 공인 데이터**를 기준으로 정확하게 계산"*,
> 간지(일진)·24절기·**절기 유래 설명**·대체휴일·음력 기념일 자동 반복·양음력 변환기·
> 날짜 계산기·**만나이(음력 생일 함께)**·홈 위젯·백업/복원·다크모드·한국어/영어/일본어.
>
> **즉 "KASI 기반 정확한 음력 달력"은 더 이상 단독 차별점이 아니다.**
> 실제로 비어 있는 급소는 아래 3개이고, 저평점 리뷰 42건에서 직접 확인했다:
>
> 1. **결제 후 광고가 다시 나온다 / 구매 복구 실패** —
>    *"결재하고 광고제거후 사용중인데.. 업데이트하더니 또 결재하라네요"*(2026-06-02),
>    *"유료로 광고없이 깔았는데 왜 광고가 나오죠?"*(2026-05-24). 개발자가 캐시 삭제·메일 문의로 대응 중.
>    → 우리 쪽 요구사항: 평생권 구매 상태를 **로컬에 영속 저장**하고, 결제 조회 실패 시
>      **광고를 켜지 말고 유료 상태를 유지**한다(실패 방향을 사용자에게 유리하게).
> 2. **앱 오프닝 전면광고** — *"앱을 열면 원치 않는 광고가 뜨고 지워지지 않는다"*(2026-08-10).
>    → 리서치 §8 광고 헌장 금지 1호. 우리는 오프닝 전면광고를 **넣지 않는다**(이미 명세대로).
> 3. **글자 크기 변경 불가** — *"구매후 사용해보니 **글자 크기 변경이 불가**한게 불편함.
>    갤럭시탭에서 가로보기로 사용하여 전체화면으로 나오지 않음"*(2025-07-28, **유료 구매자**).
>    → 우리 설정의 글자 배율 85~150% 슬라이더가 **정면 차별점**이다. 스토어 스크린샷에 반드시 노출할 것.
>    → **태블릿 가로 레이아웃**은 현재 미검증 — 출시 전 확인 항목으로 추가.
>
> **추가 확인 필요 (운영 리스크)**
> 경쟁 앱들이 **연중 수시로 공휴일 제도 변경을 반영**하고 있다 —
> `kjw.Calendar` *"노동절 법정공휴일 추가"*, `com.kydsessc.hlcfree` *"제헌절 공휴일 적용 ·
> 노동절 명칭 변경(근로자의날) · **삼복 표기 개선**"*(2026-06-16).
> 우리 공휴일 갱신 경로는 **연 1회 cron(1월 2일) + GitHub Pages 원격 JSON**인데,
> 연중 법 개정은 이 주기로 못 따라간다.
> → **워크플로우 수동 실행(workflow_dispatch) 절차를 문서화하고, 원격 JSON만 갱신하면
>   앱 업데이트 없이 반영되는지 실제로 1회 검증**해야 한다.
